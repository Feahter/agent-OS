...
const active=d.rsi.active_policy,opt=Object.values(d.optimization.active||{}),reuse=d.agent_os&&d.agent_os.reuse;document.getElementById('rsi').innerHTML=(active?`<p><span class="pill ok">路由已激活</span> ${esc(active.version)}</p><p class="muted">灰度 ${esc(active.rollout_percent)}% · ${Object.keys(active.estimates).length} 个执行器模型</p>`:'<p class="muted">尚无已激活路由策略</p>')+(opt.length?opt.map(x=>`<p><span class="pill ok">优化已激活</span> ${esc(x.kind)} · 灰度 ${esc(x.rollout_percent)}%</p>`).join(''):'<p class="muted">尚无已激活提示/拓扑优化</p>')+(reuse?`<p><span class="pill ok">安全复用</span> ${esc(reuse.entries.valid)} 个有效结果 · 节省 ${esc(reuse.saved_tokens)} Token / $${Number(reuse.saved_cost_usd||0).toFixed(4)}</p>`:'');
